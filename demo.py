...
def image_stream(imagedir, depthdir, maskdir, calib, stride, return_depth=True):
    ...
    for t, imfile in enumerate(image_list):
        ...
        if depthdir is not None:
            depth = (
                np.fromfile(os.path.join(depthdir, depth_list[t]), dtype=np.uint16)
                / 5000.0
            )  # required scaling factor
            depth = depth.reshape(
                720 // 2, 1280 // 2
            )  # aligned depth is half resolution of rgb
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="auto",
        help="Set to 'auto' to use depth from image directory. Set to 'False' or 'None' to disable depth",
    )
    parser.add_argument(
        "--maskdir",
        type=str,
    ...
    parser.add_argument(
        "--filter_thresh",
        type=float,
        default=2.4, # in case slam fails: try 1.0 here.
    ...
    # sanity checks
    rgb = os.listdir(args.imagedir)
    depth = os.listdir(args.depthdir)
    depth = [frame_name.replace(".raw", ".png") for frame_name in depth]
    mask = os.listdir(args.maskdir)
    assert len(rgb) == len(depth) == len(mask), (
        f"Expected same number of files in rgb, depth and mask dir. Got {len(rgb)}, {len(depth)}, {len(mask)}"
    )
    rgb.sort()
    depth.sort()
    mask.sort()
    assert rgb == mask, "Expected same files in rgb, depth, and mask dir."
    del rgb, depth, mask

    tstamps = []
    for t, image, intrinsics, depth in tqdm(
        image_stream(
            args.imagedir, args.depthdir, args.maskdir, args.calib, args.stride
        )
    ):
        if t < args.t0:
            continue
    ...
    traj_est = droid.terminate(
        image_stream(
            args.imagedir, None, None, args.calib, args.stride, return_depth=False
        )
    )
